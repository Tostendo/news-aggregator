    ...
    def get_undecorated_callback(self):
        ...
        while True:
            if getattr(func, '__wrapped__', False):
                ...
                def isfunc(x): return isinstance(
                    x, FunctionType) or hasattr(x, '__call__')
...
class Bottle:
    ...
    def wsgi(self, environ, start_response):
        ...
        try:
            ...
            start_response(response._wsgi_status_line(),
...
class BaseResponse:
    ...
    def headerlist(self):
        ...
        out = [(k, v.encode('utf8', 'surrogateescape').decode('latin1'))
               for (k, v) in out]
        return out
...
class MultiDict(DictMixin):
    ...
    def __init__(self, *a, **k):
        self.dict = {key: [value] for key, value in dict(*a, **k).items()}
...
class ConfigDict(dict):
    ...
    def _make_overlay(self):
        ...
        self._overlays[:] = [
            ref for ref in self._overlays if ref() is not None]
...
class _MultipartPart:
    ...
    def finish_header(self):
        ...
        self.disposition, self.options = _parse_http_header(content_disposition)[
            0]
        ...
        self.content_type, options = _parse_http_header(
            content_type)[0] if content_type else (None, {})
